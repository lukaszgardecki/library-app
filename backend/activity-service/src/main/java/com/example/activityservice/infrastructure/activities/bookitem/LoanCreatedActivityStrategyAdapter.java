...
public class LoanCreatedActivityStrategyAdapter implements ActivityCreationStrategyPort<LoanCreatedEvent> {
    ...
    public Activity create(LoanCreatedEvent event) {
        String message = event.getIsReferenceOnly()
                ? msgProvider.getMessage(MessageKey.ACTIVITY_BOOK_ITEM_BORROWED_ON_SITE, event.getBookTitle())
                : msgProvider.getMessage(MessageKey.ACTIVITY_BOOK_ITEM_BORROWED, event.getBookTitle());
        return new Activity(new UserId(event.getUserId()), ActivityType.BOOK_BORROWED, new ActivityMessage(message));
    }
