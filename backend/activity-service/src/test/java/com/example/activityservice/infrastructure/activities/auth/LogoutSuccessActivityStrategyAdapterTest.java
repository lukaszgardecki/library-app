...
class LogoutSuccessActivityStrategyAdapterTest {

    @Mock
    MessageProviderPort msgProvider;

    @InjectMocks
    LogoutSuccessActivityStrategyAdapter adapter;

    @Test
    void shouldCreateActivityFromLogoutSuccessEvent() {
        // given
        LogoutSuccessEvent event = EventTestFactory.createLogoutSuccessEvent();
        ...
        assertThat(activity.getType()).isEqualTo(ActivityType.LOGOUT);
        assertThat(activity.getMessage().value()).isEqualTo(expectedMessage);
        verify(msgProvider).getMessage(MessageKey.ACTIVITY_LOGOUT, event.getFirstName(), event.getLastName());
    }

    @Test
    void shouldSupportLogoutSuccessEvent() {
        // when
        Class<LogoutSuccessEvent> supportedClass = adapter.supports();

        // then
        assertThat(supportedClass).isEqualTo(LogoutSuccessEvent.class);
    }
}