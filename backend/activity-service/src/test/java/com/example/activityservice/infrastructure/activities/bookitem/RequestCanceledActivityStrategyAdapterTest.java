...
class RequestCanceledActivityStrategyAdapterTest {

    @Mock
    MessageProviderPort msgProvider;

    @InjectMocks
    RequestCanceledActivityStrategyAdapter adapter;

    @Test
    void shouldCreateActivityFromRequestCanceledEvent() {
        // given
        RequestCanceledEvent event = EventTestFactory.createRequestCanceledEvent();
        String expectedMessage = "testMsg";
        when(msgProvider.getMessage(MessageKey.ACTIVITY_REQUEST_CANCELLED, event.getBookTitle()))
        ...
        assertThat(activity.getType()).isEqualTo(ActivityType.REQUEST_CANCEL);
        assertThat(activity.getMessage().value()).isEqualTo(expectedMessage);
        verify(msgProvider).getMessage(MessageKey.ACTIVITY_REQUEST_CANCELLED, event.getBookTitle());
        verifyNoMoreInteractions(msgProvider);
    }

    @Test
    void shouldSupportRequestCanceledEvent() {
        // when
        Class<RequestCanceledEvent> supportedClass = adapter.supports();

        // then
        assertThat(supportedClass).isEqualTo(RequestCanceledEvent.class);
    }
}
