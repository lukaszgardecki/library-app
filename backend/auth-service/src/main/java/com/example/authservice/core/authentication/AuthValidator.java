...
class AuthValidator {
    ...
    boolean validateFgp(String token, String fingerprint) {
        String tokenFgp = extractBodyClaims(token, claims -> claims.get(CookieValueGenerator.FINGERPRINT_NAME)).toString();
        String requestFgp = TokenUtils.hashWithSHA256(fingerprint);
        return tokenFgp.equals(requestFgp);
    }

    private <T> T extractBodyClaims(String token, Function<Claims, T> claimsResolver) {
